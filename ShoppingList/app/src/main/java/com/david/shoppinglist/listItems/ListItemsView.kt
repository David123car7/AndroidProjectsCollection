...
@Composable
fun ListItemsView(modifier: Modifier, navController: NavController, firestoreDB: FirestoreDB){
...
fun ListItemsViewContent(modifier: Modifier, uiState: ListItemsViewModel.ItemsListState,
                         navController: NavController, onBuyItem:(itemBought: Item)->Unit){
        ...
        if(uiState.error != null){
            ...
            LazyColumn(modifier = Modifier.fillMaxWidth()) {
                itemsIndexed(items = uiState.items){ index, item ->
                    ItemCellView(item, onBuyItem = { onBuyItem(item) })
                }
            }
        }
...
fun ListItemsPreview(){
    ShoppingListTheme() {
        ...
        ListItemsViewContent(modifier = Modifier, navController = rememberNavController(), uiState = uiState, onBuyItem = { Unit})
    }
}
